...
class Buttons:
    ...
    for region in regions:
        select_region.insert(
            InlineKeyboardButton(text=region.name, callback_data=region.id)
        )
    ...
    def universities(region):
        ...
        for uni in universities:
            ...
            select_uni.insert(InlineKeyboardButton(text=uni.name, callback_data=uni.id))
    ...
    def areas(page):
        ...
        if len(areas) <= page * 8 + 8:
            ...
            select_area.insert(
                InlineKeyboardButton(text="Далi", callback_data=f"page_{page+1}")
            )
        if page != 0:
            select_area.add(
    ...
    def get_specs(area):
        ...
        for spec in specialities:
            select_spec.insert(
                InlineKeyboardButton(text=spec[0].name, callback_data=spec[0].name[:15])
            )
    ...
    def configure_grades(user, grades):
        ...
        for grade in grades:
            edit_grades_buttons.insert(
                InlineKeyboardButton(
                    text=grade.zno.name, callback_data=f"set_{grade.zno.id}"
                )
            )
    ...
    for zno in all_zno:
        set_grade.insert(
            InlineKeyboardButton(text=zno[1], callback_data=f"set_{zno[0]}")
        )
