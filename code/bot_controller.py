...
class Controller:
    ...
    def get_areas():
        ...
        try:
            areas = (
                session.query(Knowledge_area.name, Speciality.name)
                .filter(
                    Speciality.area_id == Knowledge_area.id,
                )
                .distinct(Knowledge_area.name)
                .all()
            )
        except:
            session.rollback()
    ...
    def get_specs(area):
        ...
        specs = (
            session.query(Speciality, Knowledge_area)
            .filter(
                Speciality.area_id == Knowledge_area.id,
                Knowledge_area.name.startswith(area),
            )
    ...
    def set_grade(tg_id, zno, grade: int):
        ...
        user_id = session.query(Users.id).filter(Users.tg_id == tg_id).first()[0]
        user_grade = (
            session.query(Grades)
            .filter(Grades.zno_id == zno, Grades.user_id == user_id)
            .first()
        )
        ...
        if not user_grade:
            if grade == 0:
                ...
                user_grade = Grades(user_id=user_id, grade=grade, zno_id=zno)
    ...
    def get_chances(tg_id, region, spec):
        ...
        specialities = (
            session.query(University, Region, Knowledge_area, Speciality)
            .filter(
                University.region_id == Region.id,
                Region.id == region,
                Knowledge_area.university_id == University.id,
                Knowledge_area.id == Speciality.area_id,
                Speciality.name.startswith(spec),
            )
            .distinct(University.name)
        )
        ...
        for spec in specialities:
            ...
            if results[1]:
                if results[0] == "additional":
                    [
                        additionals.append(zno)
                        for zno in results[1]
                        if zno not in additionals
                    ]
                else:
                    [required.append(zno) for zno in results[1] if zno not in required]
            else:
                ...
                if spec.Speciality.min_rate_budget:
                    if rate >= spec.Speciality.min_rate_budget:
                        ...
                        spec.Speciality.min_rate_budget
                        and not spec.Speciality.average_rate_contract
                    ):
                ...
                if spec.Speciality.average_rate_contract:
                    if (
                        rate >= spec.Speciality.average_rate_contract - 10
                        and spec.University.name not in budg
                    ):
                        cont.append(str(spec.University.name))
        ...
        if not budg and not cont:
            ...
            if required:
                return {"result": False, "data": list(required)}
            # If user haven't any additional zno
            elif additionals:
                return {"result": "additional", "data": list(additionals)}
        # If user have all znos, but haven't enough score
        return {"result": True, "data": {"budget": budg, "contract": cont}}
    ...
    def checking(grades, speciality_data):
        ...
        if max_nr == 0:
            ...
            return (
                "additional",
                [str(coef.zno) for coef in coefficients if not coef.required],
            )
        znos.append(max_nr)
