...
async def get_regions(message: types.Message):
    ...
    await message.answer("Оберiть ваш регiон:", reply_markup=Buttons.select_region)
...
async def add_grades(message: types.Message, state: FSMContext):
    ...
    if await state.get_state() is not None:
        await state.finish()
    await message.answer("Оберiть предмет", reply_markup=Buttons.set_grade)
...
async def additional_zno(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        if message.text == "Так":
            ...
            await message.answer(
                "Повертаємось до головного меню", reply_markup=Keyboard.home
            )
            await state.finish()
                ...
                if validate_grade(grade):
                    ...
                    if data["subjects"]:
                        ...
                        info = Controller.get_chances(
                            message.from_user.id, data["region"], data["spec"]
                        )
                        await message.answer(
...
async def calculate(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    info = Controller.get_chances(
        callback_query.from_user.id, data["region"], data["spec"]
    )
    n = "\n• "
    if info.get("result"):
...
Додайте оцiнку в меню та спробуйте ще раз.""",
            ...
            await callback_query.message.edit_text(
                result_generation(info), parse_mode=types.ParseMode.MARKDOWN
            )
    else:
        await callback_query.message.answer(
